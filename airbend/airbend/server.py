"""The optional daemon: cron-driven scheduling + webhook event intake.

`airbend serve` runs the loop; runs are executed by detached
...
def due_cron_graphs(conn):
    ...
    cutoff = (datetime.now(UTC) - _WINDOW).isoformat()
    for r in rows:
        try:
...
class _Handler(BaseHTTPRequestHandler):
    def do_POST(self) -> None:
        if self.path != "/v1/events":
            self._json(
                404,
                {"error": "not found", "help": 'POST /v1/events with {"graph": <id>}'},
            )
            return
        try:
