...
from airbend.states import TERMINAL_RUN_STATES
...
def cmd_run_events(args: argparse.Namespace) -> int:
    ...
    while True:
        ...
        if run_now is not None and run_now["status"] in TERMINAL_RUN_STATES:
            grace += 1
            if grace >= 3:
                if not printed:
                    print("events: 0 events found", end="")
    ...
    for nid, s in states.items():
        if s["state"] == "deferred":
            store.set_node_state(
                conn, run["id"], nid, "scheduled", attempt=s["attempt"] + 1, error=""
            )
            revived += 1
    if revived == 0:
        raise AirbendError(
...
def cmd_run_retry(args: argparse.Namespace) -> int:
    ...
    was_terminal = run["status"] in TERMINAL_RUN_STATES
    store.set_run_status(conn, run["id"], "running")
    if was_terminal:
        if args.watch:
...
def build_parser() -> AxiParser:
    ...
    # --- dag ---
    dag = sub.add_parser(
        "dag", help="manage graphs (DAGs)", description="Manage registered graphs."
    )
    dag.set_defaults(func=cmd_dag_help)
    dag_sub = dag.add_subparsers(
        dest="dag_command", metavar="<subcommand>", title="dag subcommands"
    )
    ...
    run_sub = run.add_subparsers(
        dest="run_command", metavar="<subcommand>", title="run subcommands"
    )
    ...
    goal_sub = goal.add_subparsers(
        dest="goal_command", metavar="<subcommand>", title="goal subcommands"
    )
