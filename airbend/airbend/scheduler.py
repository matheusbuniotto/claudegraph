...
Execution is serial: one node at a time, in declaration order among the
ready set. `max_parallel` is validated and stored but not yet acted on.
"""
...
from airbend.states import TERMINAL_NODE_STATES, TERMINAL_RUN_STATES
...
def drive_run(conn, run_id: str, stream: bool = False) -> str:
    ...
    def emit(type_: str, node_id: str | None = None, payload: dict[str, Any] | None = None) -> None:
        ...
        if stream:
            line = {"ts": store.utcnow(), "type": type_, "node_id": node_id, "payload": payload}
    ...
    def build_ctx(node: Node) -> dict[str, Any]:
        ...
        return {
            ...
            "goal": run["goal"],
            "params": params,
            "attempt": states[node.id]["attempt"],
            "channels": store.read_channels(conn, run_id),
        }

    def rollup() -> str:
        states_now = store.get_node_states(conn, run_id)
        if any(states_now[leaf]["state"] == "deferred" for leaf in leaves):
            status = "interrupted"
        elif any(states_now[leaf]["state"] == "failed" for leaf in leaves):
            status = "failed"
        else:
            status = "success"
    ...
    while True:
        ...
        if not scheduled:
            if any(states[nid]["state"] == "deferred" for nid in nodes):
                ...
                break
            if all(states[nid]["state"] in TERMINAL_NODE_STATES for nid in nodes):
                rollup()
                break
        ...
        if result.deferred:
            ...
            if attempt <= node.retries:
                ...
                if not node.on.get("failure"):
                    # No failure route: an unhandled failure ends the run.
                    store.set_run_status(conn, run_id, "failed")
                    emit("run.failed")
                    break
